...
void freeSpace(char ***names, int row, int col){
    for (int i = 0; i < row; i++){
        for (int j = 0; j < col; j++){
            free(names[i][j]);
        }
...
void handleInput(char ***names, int row, int col, int *vowelCount, int *longestNameLength, char **longestName){
...
void printNames(char ***names, int row, int col){
...
int main()
{
    ...
    handleInput(names, row, col, &vowelCount, &longestNameLength, &longestName);
    
    printNames(names, row, col);
